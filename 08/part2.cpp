...
int main(int argc, char** argv)
{
    ...
    auto last_connection = distances.end();
    for (std::size_t i = 0; i < distances.size(); i++)
    {
        ...
        if (circuits.size() == 1 && circuits[0].points.size() == points.size())
        {
            last_connection = distances.begin() + i;
            break;
        }
    }

    std::cout << "Product: " << (unsigned long) floor(last_connection->p1->x) * (unsigned long) floor(last_connection->p2->x) << '\n';
}
